...
SO3Control::SO3Control()
    : mass_(1.0), g_(9.81), thr_min_(0.0), thr_max_(1e9)
{
  acc_.setZero();
  omega_.setZero();
  old_omega_.setZero();
  omega_max_ << 7.0, 7.0, 4.0;  // roll/pitch 7 rad/s, yaw 4 rad/s
}
...
void SO3Control::setOmegaLimits(const Eigen::Vector3d &omega_max)
{
  omega_max_ = omega_max.cwiseAbs();  // 取绝对值，防止传负数进来
}

void SO3Control::setThrustLimits(const double thr_min, const double thr_max)
{
  thr_min_ = thr_min;
  thr_max_ = thr_max;
}
                                  ...
                                  quadrotor_msgs::Px4ctrlDebug &debug_)
{
  ...
  double x_vel_error = std::max(std::min(des_vel(0) - vel_(0), 1.0), -1.0);
  acc_error(0) = kx(0) * x_pos_error + kv(0) * x_vel_error;

  // y acceleration
  double y_pos_error = std::isnan(des_pos(1)) ? 0.0 : std::max(std::min(des_pos(1) - pos_(1), 1.0), -1.0);
  double y_vel_error = std::max(std::min(des_vel(1) - vel_(1), 1.0), -1.0);
  acc_error(1) = kx(1) * y_pos_error + kv(1) * y_vel_error;

  // z acceleration
  double z_pos_error = std::isnan(des_pos(2)) ? 0.0 : std::max(std::min(des_pos(2) - pos_(2), 1.0), -1.0);
  double z_vel_error = std::max(std::min(des_vel(2)  - vel_(2), 1.0), -1.0);
  acc_error(2) = kx(2) * z_pos_error + kv(2) * z_vel_error;
  
  debug_.des_v_x = (des_vel(0) + kx(0) * x_pos_error); //debug
  debug_.des_v_y = (des_vel(1) + kx(1) * y_pos_error);
  debug_.des_v_z = (des_vel(2) + kx(2) * z_pos_error);
  
  debug_.dir_x = des_dir(0);
  debug_.dir_y = des_dir(1);
  debug_.dir_z = des_dir(2);

  debug_.dir_dot_x = des_dir_dot(0);
  ...
  // if (forward(des_vel, des_acc, des_jer, des_dir, des_dir_dot, yaw_, yaw_dot_, thr_, quat, omega_))
  {
    ...
    Eigen::Vector3d rotvec = rotation_vector.axis() * rotation_vector.angle();

    debug_.err_axisang_x = rotvec(0);
    debug_.err_axisang_y = rotvec(1);
    debug_.err_axisang_z = rotvec(2);
    debug_.err_axisang_ang = rotation_vector.angle();
    ...
    double sx = (omega_max_.x() > 0.0) ? std::abs(omega_.x()) / omega_max_.x() : 0.0;
    double sy = (omega_max_.y() > 0.0) ? std::abs(omega_.y()) / omega_max_.y() : 0.0;
    double sz = (omega_max_.z() > 0.0) ? std::abs(omega_.z()) / omega_max_.z() : 0.0;

    double s = std::max({sx, sy, sz, 1.0});  // 取最大超限比例，至少为1
