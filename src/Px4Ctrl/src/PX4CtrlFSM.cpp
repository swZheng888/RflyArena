...
void PX4CtrlFSM::process()
{
	...
	case MANUAL_CTRL:
	{
		...
		else if (param.takeoff_land.enable && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::TAKEOFF) // Try to jump to AUTO_TAKEOFF
		{
			...
			ROS_INFO("\033test1\033[32m");

			state = AUTO_TAKEOFF;
			controller.resetThrustMapping();
			ROS_INFO("\033test2\033[32m");

			set_start_pose_for_takeoff_land(odom_data);
			toggle_offboard_mode(true);				  // toggle on offboard before arm
			ROS_INFO("\033test3\033[32m");
	...
	case AUTO_HOVER:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			...
			ROS_INFO("\033[32m[px4ctrl] AUTO_HOVER(L2) --> MANUAL_CTRL(L1)\033[32m");
		}
	...
	{
		// 智能控制器选择：检测是否为纯位置指令（速度和加速度都接近0）
		bool is_position_only_cmd = (des.v.norm() < 0.01 && des.a.norm() < 0.01);
		int effective_solver = param.pose_solver;
		
		// 静态变量跟踪上次使用的控制器
		static int last_effective_solver = -1;
		
		// 如果配置为SO3控制器(3)但收到纯位置指令，自动切换到位置控制模式(1)
		if (param.pose_solver == 3 && is_position_only_cmd)
		{
			effective_solver = 1;  // 使用alg1位置控制
		}
		
		// 打印控制器切换日志
		if (effective_solver != last_effective_solver && last_effective_solver != -1)
		{
			if (effective_solver == 1)
			{
				ROS_INFO("[px4ctrl] Switching to position control mode (alg1) - position-only command detected");
			}
			else if (effective_solver == 3)
			{
				ROS_INFO("[px4ctrl] Switching to SO3 trajectory control mode (alg3) - full trajectory command detected");
			}
		}
		last_effective_solver = effective_solver;
...
Desired_State_t PX4CtrlFSM::get_hover_des()
{
	...
	des.des_dir_ = Eigen::Vector3d(cos(des.yaw),sin(des.yaw),0);
	des.des_dir_dot_ = Eigen::Vector3d(0,0,0);
	des.des_yaw_dot_ = 0;
	des.des_yaw_ = 0.03;
	return des;
}
...
Desired_State_t PX4CtrlFSM::get_cmd_des()
{
	...
	// des.des_dir_ = Eigen::Vector3d(cos(des.yaw),sin(des.yaw),0);
	// des.des_dir_dot_ = Eigen::Vector3d(0,0,0);
	des.des_dir_ = cmd_data.des_dir_;
	
	des.des_dir_dot_ = cmd_data.des_dir_dot_;
	des.des_yaw_ = cmd_data.des_yaw_;
	des.des_yaw_dot_ = cmd_data.des_yaw_dot_;
...
Desired_State_t PX4CtrlFSM::get_rotor_speed_up_des(const ros::Time now)
{
	...
	des.des_dir_dot_ = Eigen::Vector3d(0,0,0);
	des.des_yaw_dot_ = 0;
	des.des_yaw_ = takeoff_land.start_pose(3);
