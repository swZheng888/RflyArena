...
RC_Data_t::RC_Data_t()
{
    ...
    is_command_mode = true;  // Changed to true for RC-free simulation usage
...
void Odom_Data_t::feed(nav_msgs::OdometryConstPtr pMsg)
{
    ...
    if (is_first_msg_)
    {
        // 如果是第一帧，无法微分，只能初始化
        last_position_ = p;
        last_time_ = now; // 建议用 pMsg->header.stamp
        // v = Eigen::Vector3d::Zero(); // 初始速度设为0
        is_first_msg_ = false;
        return;
    }
    // 2. 计算时间差 dt
    ...
    // 保护：防止 dt 为 0 或过大（例如丢包）导致速度计算错误
    if (dt > 0.001 && dt < 1.0) 
    {
        // 3. 有限差分：计算“粗糙”的瞬时速度 (Raw Velocity)
        // v_raw = (当前位置 - 上次位置) / dt
        Eigen::Vector3d v_raw = (p - last_position_) / dt;

        // 4. 低通滤波 (Low Pass Filter)
        // 公式: v_new = alpha * v_raw + (1 - alpha) * v_old
        // v 在这里既存储了上一帧的平滑速度，也被更新为当前帧的平滑速度
        // v = alpha_ * v_raw + (1.0 - alpha_) * v;
        
        // 如果您想保留 Z 轴的原始速度（有些动捕Z轴速度还行），可以单独注释掉 Z 轴的覆盖
        // v(2) = v_raw(2); // 或者保留原始测量值
    }
    else
    {
        // 如果时间间隔异常，保持上一次的速度或设为0
        // v = Eigen::Vector3d::Zero(); 
        // ROS_WARN("Odom dt exception: %f", dt);
    }
...
void Command_Data_t::feed(quadrotor_msgs::PositionCommandConstPtr pMsg)
{
    ...
    des_dir_ = Eigen::Vector3d(msg.yaw_dir.x, msg.yaw_dir.y, msg.yaw_dir.z);
    des_dir_dot_ = Eigen::Vector3d(msg.yaw_dir_dot.x, msg.yaw_dir_dot.y, msg.yaw_dir_dot.z);
