...
Controller::Controller(Parameter_t &param_) : param(param_)
{
  ...
  controller_.setOmegaLimits(omega_limit);
  resetThrustMapping();
    ...
    double voltage)
{
  ...
  // ROS_ERROR("des pos: %lf, %lf, %lf", des.p(0), des.p(1), des.p(2));
  // ROS_ERROR("des vel: %lf, %lf, %lf", des.v(0), des.v(1), des.v(2));
  ...
  // ROS_ERROR("des acc: %lf, %lf, %lf", total_des_acc(0), total_des_acc(1), total_des_acc(2));
  // ROS_ERROR("imu acc: %lf, %lf, %lf", imu.a(0), imu.a(1), imu.a(2));
  ...
  // ROS_WARN("des thrust: %lf, %lf, %lf", u.thrust, total_des_acc.norm(), thr2acc);
  ...
  // ROS_WARN("des state: %lf", des.v(2));
  // ROS_ERROR("total des acc: %lf", total_des_acc(2));
  const Eigen::Vector3d feedback_bodyrates = computeFeedBackControlBodyrates(desired_attitude, odom.q, param);

  // ROS_WARN("feedback bodyrates: %lf, %lf, %lf, u bodyrates: %lf, %lf, %lf", feedback_bodyrates(0), 
  //                        feedback_bodyrates(1), feedback_bodyrates(2), u.bodyrates(0), u.bodyrates(1), u.bodyrates(2));
  ...
  // Eigen::Vector3d Euler = desired_attitude.toRotationMatrix().eulerAngles(0, 1, 2);
  // debug.des_q_w = Euler(0);
  // debug.des_q_x = Euler(1);
  // debug.des_q_y = Euler(2);

  // while(debug.des_q_w > M_PI)
  //   debug.des_q_w -= 2 * M_PI;
  // while(debug.des_q_w < -M_PI)
  //   debug.des_q_w += 2 * M_PI;

  // while(debug.des_q_y > M_PI)
  //   debug.des_q_y -= 2 * M_PI;
  // while(debug.des_q_y < -M_PI)
  //   debug.des_q_y += 2 * M_PI;
  
  // while(debug.des_q_x > M_PI)
  //   debug.des_q_x -= 2 * M_PI;
  // while(debug.des_q_x < -M_PI)
  //   debug.des_q_x += 2 * M_PI;
    ...
    double voltage)
{
  ...
  controller_.setAcc(imu.a);
  
  debug.odom_v_x = odom.v(0); //debug
  debug.odom_v_y = odom.v(1);
  debug.odom_v_z = odom.v(2);

  debug.des_p_x = des.p(0); //debug
  debug.des_p_y = des.p(1);
  debug.des_p_z = des.p(2);

  debug.odom_p_x = odom.p(0); //debug
  debug.odom_p_y = odom.p(1);
  debug.odom_p_z = odom.p(2);

  debug.odom_q_x = odom.q.x(); //debug
  debug.odom_q_y = odom.q.y();
  debug.odom_q_z = odom.q.z();
  debug.odom_q_w = odom.q.w();

  debug.des_a_x = des.a(0);
  debug.des_a_y = des.a(1);
  debug.des_a_z = des.a(2);
  
  controller_.calculateControl(des.p, des.v, des.a, des.j, des.des_dir_,
                               des.des_dir_dot_, des.des_yaw_, des.des_yaw_dot_, Kp, Kv, KAng, debug);
  u.q = imu.q * odom.q.inverse() * controller_.getComputedOrientation();  
  u.bodyrates = controller_.getComputedOmega();
  // std::cout<<"u.bodyrates:"<<u.bodyrates(0)<<":"<<u.bodyrates(1)<<":"<<u.bodyrates(2)<<endl;
  double thrust = controller_.getThrust();
  
  debug.des_q_x = controller_.getComputedOrientation().x(); //debug
  debug.des_q_y = controller_.getComputedOrientation().y();
  debug.des_q_z = controller_.getComputedOrientation().z();
  debug.des_q_w = controller_.getComputedOrientation().w();
  // double C_R = 1500.0;  // 转速与油门的比例系数
  // double w_b = 151.72;  // 零油门时的基准转速
  // // 根据推力公式 F = C_T * N^2
  // double C_T = 1.222e-5;  // 推力系数
  // double N = sqrt(thrust /(4* C_T));  // 计算所需的转速
  // // 使用转速和油门的映射关系 w = C_R * 油门 + w_b
  // double throttle = (N - w_b) / C_R;

  // u.thrust = thrust / thr2acc;
  u.thrust = (thrust/4 +0.7873) /3.8619;
  // u.thrust = thrust /(9.8/0.38);
  // u.thrust = throttle;
  debug.des_thr = thrust;
  debug.throttle = u.thrust;
  return debug; //debug
    ...
    const Parameter_t &param)
{
  ...
  // ROS_INFO("des_q: %lf, %lf, %lf, %lf", des_q.w(), des_q.x(), des_q.y(), des_q.z());
  // ROS_WARN("est_q: %lf, %lf, %lf, %lf", est_q.w(), est_q.x(), est_q.y(), est_q.z());
    ...
    const Parameter_t &param)
{
  ...
  double x_vel_error = std::max(std::min(des.v(0) - odom.v(0), 1.0), -1.0);
  acc_error(0) = Kp(0) * x_pos_error + Kv(0) * x_vel_error;

  // y acceleration
  double y_pos_error = std::isnan(des.p(1)) ? 0.0 : std::max(std::min(des.p(1) - odom.p(1), 1.0), -1.0);
  double y_vel_error = std::max(std::min(des.v(1) - odom.v(1), 1.0), -1.0);
  acc_error(1) = Kp(1) * y_pos_error + Kv(1) * y_vel_error;

  // z acceleration
  double z_pos_error = std::isnan(des.p(2)) ? 0.0 : std::max(std::min(des.p(2) - odom.p(2), 1.0), -1.0);
  double z_vel_error = std::max(std::min(des.v(2)  - odom.v(2), 1.0), -1.0);
  acc_error(2) = Kp(2) * z_pos_error + Kv(2) * z_vel_error;
  // x acceleration
  // double x_pos_error = std::isnan(des.p(0)) ? 0.0 : std::max(std::min(des.p(0) - odom.p(0), 1.0), -1.0);
  ...
  // ROS_INFO("Pos error: %lf, %lf, %lf", x_pos_error, y_pos_error, z_pos_error);
  // ROS_INFO("vel error: %lf, %lf, %lf", des.v(0) - odom.v(0), des.v(1) - odom.v(1), des.v(2) - odom.v(2));
  // ROS_INFO("Vel error: %lf, %lf, %lf", x_vel_error, y_vel_error, z_vel_error);
  // ROS_INFO("Acc error: %lf, %lf, %lf", acc_error(0), acc_error(1), acc_error(2));
    ...
    const Eigen::Vector3d &drag_acc /*default = Eigen::Vector3d::Zero() */) const
{
  ...
  // ROS_INFO("PID_ERROR_ACC: %lf, %lf, %lf, ref_ACC: %lf, %lf, %lf", 
    // PIDErrorAcc(0), PIDErrorAcc(1), PIDErrorAcc(2), ref_acc(0), ref_acc(1), ref_acc(2));
    ...
    double voltage)
{
  ...
  // ROS_WARN("des trust: %lf, %lf, %lf, %lf", est_q.w(), body_z_axis(0), body_z_axis(1), body_z_axis(2));
  ...
  {
    // 使用与alg3相同的推力映射公式，确保控制器切换时连续
    // thrust = mass * des_acc_norm (这里des_acc_norm已经包含了重力补偿)
    double thrust = param.mass * des_acc_norm;
    normalized_thrust = (thrust/4 + 0.7873) / 3.8619;
  }
  // ROS_WARN("des trust: %lf, %lf, %lf", normalized_thrust, des_acc_norm, thr2acc);
    ...
    const Parameter_t &param)
{
  ...
  while (timed_thrust.size() >= 1)
  {
    ...
    else
    {
      ...
      // ROS_INFO("thr2acc: %lf", thr2acc);
      thr2acc = thr2acc + K * (est_a(2) - thr * thr2acc);
      // ROS_INFO("Thrust2acc: %lf, P: %lf, est_a: %lf, res: %lf", thr2acc, P, est_a(2), thr*thr2acc);
      P = (1 - K * thr) * P / rho2;
      // printf("%6.3f,%6.3f,%6.3f,%6.3f\n", thr2acc, gamma, K, P);
      //fflush(stdout);
      const double hover_percentage = param.gra / thr2acc;
      if ( hover_percentage > 0.99 || hover_percentage < 0.01 )
      {
        ROS_ERROR("Estimated hover_percentage >0.9 or <0.01! Perhaps the accel vibration is too high!");
        thr2acc = hover_percentage > 0.99 ? param.gra / 0.99 : thr2acc;
        thr2acc = hover_percentage < 0.01 ? param.gra / 0.01 : thr2acc;
      }
